...
import commercialsFakerFactory from "./commercialsFakerFactory";
...
const CommercialsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = commercialsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("commercials").create(elem));
...
export default connect(mapState, mapDispatch)(CommercialsFakerDialogComponent);
