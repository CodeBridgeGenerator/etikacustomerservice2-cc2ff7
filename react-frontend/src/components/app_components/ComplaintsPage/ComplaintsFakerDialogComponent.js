...
import complaintsFakerFactory from "./complaintsFakerFactory";
...
const ComplaintsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = complaintsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("complaints").create(elem));
...
export default connect(mapState, mapDispatch)(ComplaintsFakerDialogComponent);
