...
import refundsFakerFactory from "./refundsFakerFactory";
...
const RefundsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refundsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refunds").create(elem));
...
export default connect(mapState, mapDispatch)(RefundsFakerDialogComponent);
